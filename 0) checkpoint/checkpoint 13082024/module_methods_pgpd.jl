...
function instantiate_system_QCRM_pgpd(data_model)
    ...
    # added
    load_indices = sort(unique(map(x -> x["load_bus"], values(pm.data["load"]))))
    counter = 1
    for d in load_indices
        push!(vars, JuMP.variable_by_name(pm.model, string("0_pd[",counter,"]")))
        counter += 1
    end
...
function dim_and_limits_variables_pgpd(network_data)
    ndim = 0
    bus_gen = []
    gen_index = []
    bus_load = []
    ...
    # end
    for i in load_index
        push!(min_lim, 0.8*network_data["load"]["$i"]["pd"]) # *0.6
        push!(max_lim, network_data["load"]["$i"]["pd"]) # *1
    end
...
function dim_and_limits_variables_init(network_data)
    ndim = 0
    bus_gen = []
    gen_index = []
    bus_load = []
    ...
    return ndim, bus_gen, gen_index, load_index, min_lim, max_lim
end 
...
function infeasibility_certif_constraints_check_pgpd(network_data , Nb_HP, Nb_insamples)
    ...
    #------------------ choose to create, store (and create) or load polytope -------
    polytope_method = "create"
    ...
    ndim, _, _, _, min_lim, max_lim = dim_and_limits_variables_init(data_tight_tmp)
    ...
    sample_volt = sample_ops[:, 6:11] # 6:11
    sample_ops = hcat(sample_ops[:, 1:5], sample_ops[:, 12:end])
    ...
    ndim, _, _, min_lim, max_lim = dim_and_limits_variables_pgpd(data_tight_tmp)
    create_scaled_pol(ndim, min_lim , max_lim)
    _, Ainit, binit = get_full_pol()
    v = comp_vol()
    ...
    for i=1:length(pm.var[:it][:pm][:nw][0][:pg].data)
        if i != slack_gen_idx && data_tight_tmp["gen"]["$i"]["pmax"] > 0.0
            push!(vars, JuMP.variable_by_name(pm.model, string("0_pg[",i,"]")))
            #push!(gen_vars, JuMP.variable_by_name(pm.model, string("0_pg[",i,"]")))
        end
    end

    # set initial values to voltage variables
    gen_indexes = (unique(map(x -> x["gen_bus"], values(pm.data["gen"]))))
    for g in gen_indexes
        push!(volt_vars, JuMP.variable_by_name(pm.model, string("0_vm[",g,"]")))
    end

    # added
    load_indices = sort(unique(map(x -> x["load_bus"], values(pm.data["load"])))) # "load_bus"
    counter = 1
    for d in load_indices
        #push!(load_vars, JuMP.variable_by_name(pm.model, string("0_pd[",counter,"]")))
        push!(vars, JuMP.variable_by_name(pm.model, string("0_pd[", counter, "]")))
        counter += 1
    end
    ...
    Nvolt = length(volt_vars)
    x_volt = sample_volt[1,:]
    @variable(pm.model, aux_volt[1:Nvolt])
    @constraint(pm.model, aux_volt .== volt_vars)
    @NLparameter(pm.model, x_volt_p[i = 1:Nvolt] == x_volt[i])

    for i in 1:Nvolt
        @NLconstraint(pm.model, (1 - 0.02) * x_volt_p[i] <= aux_volt[i])
        @NLconstraint(pm.model, aux_volt[i]  <= x_volt_p[i] * (1 + 0.02))
    end

    #@NLconstraint(pm.model, con_sphere, sqrt(sum((aux[i]-x_hat_p[i])^2 for i in 1:N) + sum((aux_volt[i]-x_volt_p[i])^2 for i in 1:Nvolt)) <= r) # add constraint on radius
    ...
    HP_time = @elapsed begin
        if polytope_method == "create" || polytope_method == "store"
            ...
            for j in 2:length(sample_ops[:,1]) 
                ...
                if result["termination_status"] == LOCALLY_SOLVED # check if solved
                    ...
                    r_opt = 0
                    #print_summary(result["solution"])
                    #print(println("x_hat: ", x_hat))
                end

                if !(isapprox(JuMP.value(r), 0; atol= 1e-4)) && (r_opt > 0) # if initial point has positive radius and is not zero
                    ...
                    A = vec(n_normT/normal_normalized)
                ...
                global x_volt = sample_volt[j,:]
                JuMP.set_value.(x_volt_p, x_volt)
    ...
    if polytope_method == "store"
        _, A, b = get_full_pol()
        #print(P)

        df_A = DataFrame(A, :auto)
        df_b = DataFrame(b=b)

        # Save to CSV
        CSV.write("polytope_A.csv", df_A)
        CSV.write("polytope_b.csv", df_b)
    end

    #------------- reconstruct polytope

    if polytope_method == "load"
        # Load A and b from CSV files
        df_A = CSV.read("polytope_A.csv", DataFrame)
        df_b = CSV.read("polytope_b.csv", DataFrame)

        # Convert DataFrames to matrices/vectors
        A = Matrix(df_A)
        b = df_b.b

        create_scaled_pol(ndim, min_lim , max_lim)
        for i in length(binit):length(b)
            Arow = A[i,:]
            brow = b[i,:]
            add_to_pol(Arow, brow)
        end

        v = comp_vol()
        println("Loaded polytope volume : ", v)
    end

    # Pnew, Anew, bnew = get_full_pol()
    # print(Pnew)
    ...
    pm, N, vars, header = instantiate_system_QCRM_pgpd(data_tight_tmp)
    ...
    sampling_time = @elapsed begin
        for i in 1:nb_samples # loop over number of samples in polytope
            ...
            for d in eachindex(pd_numbers)
                data_opf_verif["load"]["$(pd_numbers[d])"]["pd"] = in_samples[length(pg_numbers)+d,i] # +length(vm_numbers)
            ...
            if PF_res2["termination_status"] == LOCALLY_SOLVED || acpfcorrect_feasibility == true 
                ...
                if PF_res2["termination_status"] == LOCALLY_SOLVED
            ...
            #     # solve pvpq
            #     PF_res1 = adjust_PVPQ(data_opf_verif, 2) # changed this to only 2 iterations
...
function N_1_step(network_data, OPs_Feas, cont_out)
    ...
    pm, N, vars, header = instantiate_system_QCRM_pgpd(network_data)
    pg_numbers, vm_numbers, pd_numbers = extract_number_and_type(vcat(header[1]))
...
function sample_MVND(OP_HP, network_basic, data_tight, Nb_ops)
    ...
    pm, N, vars, header = instantiate_system_QCRM_pgpd(data_tight)
    pg_numbers, vm_numbers, pd_numbers = extract_number_and_type(vcat(header[1]))
    pf_results = []
    load_results = []
