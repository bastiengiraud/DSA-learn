...
function instantiate_system_QCRM_pgvm(data_model)
    pm = instantiate_model(data_model, QCRMPowerModel, PowerModels.build_opf) # PowerModels.build_opf)
    
    # for var in all_variables(pm.model)
...
function dim_and_limits_variables_pgvm(network_data)
    ndim = 0
    bus_gen = []
    gen_index = []
    bus_load = []
...
function infeasibility_certif_constraints_check_pgvm(network_data , Nb_HP, Nb_insamples)
    ...
    data_tight_tmp, _ = solve_obbt_opf!(network_data, optimizer_with_attributes(Ipopt.Optimizer, "print_level" => 0), max_iter=iter, model_constructor=QCLSPowerModel)
    
    ndim, _, _, min_lim, max_lim = dim_and_limits_variables_pgvm(data_tight_tmp)
    ...
    pm, N, vars, header = instantiate_system_QCRM_pgvm(data_tight_tmp)
    ...
    sampling_time = @elapsed begin
        for i in 1:nb_samples # loop over number of samples in polytope
            ...
            if PF_res0["termination_status"] == LOCALLY_SOLVED && initial_feasibility == true 
                ...
                # solve pvpq
                PF_res1 = adjust_PVPQ(data_opf_verif, 2)
                ...
                if PF_res1["termination_status"] == LOCALLY_SOLVED && PVPQ_feasibility == true 
                    ...
                    vars_new = []
                    pm = instantiate_model(data_opf_verif, ACPPowerModel, PowerModels.build_opf)
...
function N_1_step(network_data, OPs_Feas, cont_out)
    ...
    pm, N, vars, header = instantiate_system_QCRM_pgvm(network_data)
...
function sample_MVND(OP_HP, network_basic, data_tight, Nb_ops)
    ...
    pm, N, vars, header = instantiate_system_QCRM_pgvm(data_tight)
    pg_numbers, vm_numbers, pd_numbers = extract_number_and_type(vcat(header[1]))
    pf_results = []
    load_results = []
